...
namespace Cometary
{
    ...
    internal sealed class Proxy
    {
        ...
        public object Invoke(string name, params object[] args) => TryInvoke(name, args, out object result)
            ? result
            : throw new InvalidOperationException();
        public T Invoke<T>(string name, params object[] args) => (T)Invoke(name, args);

        public object Get(string name) => TryGet(name, out object result) ? result : throw new InvalidOperationException();
        ...
        public bool TryInvoke(string name, object[] args, out object result)
        {
            ...
            if (data.Invokers.TryGetValue(key, out Func<object, object[], object> del))
            {
                result = del(obj, args);
                return true;
            }
        ...
        public bool TryGet(string name, out object result)
        {
            ...
            if (data.Getters.TryGetValue(key, out Func<object, object> del))
            {
                result = del(obj);
                return true;
            }
        ...
        public bool TrySet(string name, object value)
        {
            ...
            if (data.Setters.TryGetValue(key, out Func<object, object, object> del))
            {
                del(Object, value);
                return true;
            }
